...
必要な変数名は `example.env` と文書だけから確認し、値は GitHub Environments、Cloudflare secrets、ブラウザストアの secrets で管理します。
...
- `pnpm dev`: 拡張機能と Worker の開発タスクを開始します。
...
- `pnpm build`: 全ワークスペースを依存順にビルドします。
- `pnpm check`: format、lint、typecheck をまとめて確認します。
- `pnpm validate`: format、lint、typecheck、test、build を一通り確認します。

依存を変更したら `pnpm install --frozen-lockfile=false` で lockfile を同期し、完了前は `pnpm validate` を実行します。
